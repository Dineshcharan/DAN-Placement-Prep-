...
import { ArrowLeft, LogOut, FileCode, Target, CheckCircle2, TrendingUp } from 'lucide-react';
...
export default function Pseudocodes() {
  const navigate = useNavigate();
  const { signOut, user } = useAuth();
  const [algorithmStats, setAlgorithmStats] = useState({ total: 100, completed: 0, correct: 0 });
  const [generalStats, setGeneralStats] = useState({ total: 100, completed: 0, correct: 0 });

  useEffect(() => {
    const loadStats = async () => {
      if (!user) return;

      // Load algorithm pseudocodes stats
      ...
      if (algorithmData) {
        setAlgorithmStats({
          total: 100,
          completed: algorithmData.length,
          correct: algorithmData.filter(p => p.is_correct).length,
        });
      }

      // Load general pseudocodes stats
      ...
      if (generalData) {
        setGeneralStats({
          total: 100,
          completed: generalData.length,
          correct: generalData.filter(p => p.is_correct).length,
        });
      }
    };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Watermark />
      <header className="border-b bg-gradient-to-r from-violet-500 to-purple-500 shadow-sm">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-white">Pseudocodes</h1>
          </div>
          <div className="flex items-center gap-3">
            <ThemeToggle />
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        <div className="mb-8 animate-fade-in">
          <h2 className="text-3xl font-bold mb-2">Pseudocodes Practice</h2>
          <p className="text-muted-foreground">
            Master algorithms and code execution with 200+ practice problems
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <Card 
            ...
            onClick={() => navigate('/pseudocodes-algorithm')}
          >
            <CardHeader>
              <div className="flex items-center gap-3 mb-2">
                ...
                <CardTitle className="text-xl">Algorithm Pseudocodes</CardTitle>
              </div>
              <CardDescription>
                100 essential algorithms with detailed pseudocodes. Learn recursive algorithms, searching, sorting, and more.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center p-3 bg-muted rounded-lg">
                  ...
                  <div className="text-2xl font-bold">{algorithmStats.total}</div>
                ...
                <div className="text-center p-3 bg-muted rounded-lg">
                  ...
                  <div className="text-2xl font-bold">{algorithmStats.completed}</div>
                ...
                <div className="text-center p-3 bg-muted rounded-lg">
                  ...
                  <div className="text-2xl font-bold">
                    {algorithmStats.completed > 0 
                      ? Math.round((algorithmStats.correct / algorithmStats.completed) * 100)
                      : 0}%
                  </div>
          ...
          <Card 
            className="hover:shadow-glow transition-all duration-300 cursor-pointer hover:-translate-y-1"
            onClick={() => navigate('/pseudocodes-general')}
          >
            <CardHeader>
              <div className="flex items-center gap-3 mb-2">
                <div className="bg-gradient-primary rounded-full p-2">
                ...
                <CardTitle className="text-xl">General Pseudocodes</CardTitle>
              </div>
              <CardDescription>
                100 code execution trace questions from real placement exams. Practice loops, conditions, and logic.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="text-center p-3 bg-muted rounded-lg">
                  ...
                  <div className="text-2xl font-bold">{generalStats.completed}</div>
                  <div className="text-xs text-muted-foreground">Completed</div>
                </div>
                <div className="text-center p-3 bg-muted rounded-lg">
                  <div className="flex items-center justify-center gap-1 mb-1">
                  ...
                  <div className="text-2xl font-bold">
                    {generalStats.completed > 0 
                      ? Math.round((generalStats.correct / generalStats.completed) * 100)
                      : 0}%
                  </div>
                  <div className="text-xs text-muted-foreground">Success</div>
                </div>
