...
- **10 Categories**: Array (9 problems), Binary (5), Dynamic Programming (11), Graph (8), Interval (6), Linked List (6), Matrix (4), String (10), Tree (14), Heap (2)
...
- Averages, Ratios & Proportions, Number Systems, Series
- Simple & Compound Interest, Data Interpretation
- Alligation & Mixture, Probability, Permutations & Combinations, Geometry & Mensuration, Logarithms
- **15 challenging questions per topic** with detailed explanations after submission
- Every question comes with a step-by-step explanation so students can learn the solving approach

### 2. Reasoning
Develop logical reasoning skills with **advanced, company-aptitude-level** questions — crafted to simulate real placement test difficulty:
- Blood Relations, Coding-Decoding, Seating Arrangements
- Direction Sense, Syllogisms, Puzzles, Pattern Recognition, Analogies
- Input-Output, Number Series, Statement & Conclusions, Data Sufficiency, Clocks & Calendars, Cubes & Dice, Order & Ranking
- **15 challenging questions per topic** with detailed explanations after submission
- Each question includes a thorough explanation to build strong reasoning fundamentals
...
### 7. Technical Questions ⭐ UPGRADED!
Prepare for technical interviews with **50 unique, interview-level questions per subject** (100 total per section) — all questions are commonly asked in real company interviews:
- **OOPS**: SOLID principles, design patterns (Singleton, Factory, Observer, Strategy, Decorator), vtable mechanism, object slicing, diamond problem, shallow vs deep copy, RAII, composition vs inheritance, covariant return types, and more
- **DBMS**: B+ Tree indexing, query optimization, execution plans, MVCC, sharding, CAP theorem, window functions, N+1 problem, OLTP vs OLAP, transaction isolation levels, deadlock prevention, and more
- **OS**: Banker's algorithm, page replacement (FIFO, LRU, Optimal), producer-consumer problem, readers-writers problem, dining philosophers, TLB, copy-on-write, zombie/orphan processes, CFS scheduler, and more
- **Computer Networks**: TCP 3-way handshake, HTTP/1.1 vs HTTP/2 vs HTTP/3, TLS handshake, DNS resolution, subnetting, VLAN, BGP, congestion control (Slow Start, AIMD), WebSocket, SDN, and more
- **Cloud Computing**: 100 interview-level questions covering IaaS/PaaS/SaaS, AWS/Azure/GCP, Kubernetes, Docker, DevOps, CI/CD, serverless, disaster recovery, and more
...
- "I'm so glad you're exploring our platform! Here's what you need to know..."
...
serve(async (req) => {
  ...
  try {
    ...
    let systemContext = WEBSITE_CONTEXT;
    if (userName) {
      systemContext += `\n\n## Current User\nThe user's name is "${userName}". Address them by their name occasionally to make the conversation warm and personal. Be extra polite and helpful.`;
    } else {
      systemContext += `\n\n## Current User\nThe user is not logged in or their name is not available. Still be warm and polite, and encourage them to sign up or log in to access all features.`;
    }
